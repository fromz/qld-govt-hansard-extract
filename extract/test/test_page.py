...
class TestPage(unittest.TestCase):
    def setUp(self):
        self.page = Page({})
        tl = TextLine(BBox(Coordinate(79.2,793.923), Coordinate(139.809,807.518)))
        tl.texts.append(Text({'bbox': '79.2,793.923,139.809,807.518'}, 'test'))
        tb = TextBox()
        tb.text_lines.append(tl)
        self.page.text_boxes.append(tb)

        tl = TextLine(BBox(Coordinate(70,793.923), Coordinate(354.369,807.518)))
        tl.texts.append(Text({'bbox': '70,793.923,354.369,807.518'}, 'test'))
        tb = TextBox()
        tb.text_lines.append(tl)
        self.page.text_boxes.append(tb)

        tl = TextLine(BBox(Coordinate(505.2,793.923), Coordinate(530.169,807.518)))
        tl.texts.append(Text({'bbox': '505.2,793.923,530.169,807.518'}, 'test'))
        tb = TextBox()
        tb.text_lines.append(tl)
        self.page.text_boxes.append(tb)
